-- NYC Real Estate AI - CLEAN MIGRATION
...
CREATE TABLE neighborhoods (
    ...
    rent_to_price_ratio NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN median_price > 0 THEN ROUND((median_rent * 12) / median_price * 100, 2) ELSE 0 END
    ) STORED
);
...
CREATE TABLE properties (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    price_cut_count INTEGER GENERATED ALWAYS AS (
        CASE WHEN price_history IS NOT NULL THEN jsonb_array_length(price_history) - 1 ELSE 0 END
    ) STORED,
    total_cut_percent NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN original_price > 0 AND current_price > 0
        THEN ROUND((original_price - current_price) / original_price * 100, 2) ELSE 0 END
    ) STORED,
    price_per_sqft NUMERIC(10,2) GENERATED ALWAYS AS (
        CASE WHEN sqft > 0 THEN ROUND(current_price / sqft, 2) ELSE 0 END
    ) STORED,
    hoa_percent_of_price NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN current_price > 0 THEN ROUND((monthly_hoa * 12) / current_price * 100, 2) ELSE 0 END
    ) STORED,
    annual_rent NUMERIC(12,2) GENERATED ALWAYS AS (estimated_monthly_rent * 12) STORED,
    rent_to_price_ratio NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN current_price > 0 THEN ROUND((estimated_monthly_rent * 12) / current_price * 100, 2) ELSE 0 END
    ) STORED,
    appreciation_since_last_sale NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN last_sale_price > 0 AND current_price > 0
        THEN ROUND((current_price - last_sale_price) / last_sale_price * 100, 2) ELSE 0 END
    ) STORED,
    distress_flag TEXT GENERATED ALWAYS AS (
        CASE
            WHEN total_cut_percent >= 10 AND price_cut_count >= 2 AND days_on_market >= 60 THEN 'High'
            WHEN total_cut_percent >= 5 OR price_cut_count >= 1 OR days_on_market >= 45 THEN 'Medium'
            ELSE 'Low'
        END
    ) STORED,
    total_annual_expenses NUMERIC(12,2) GENERATED ALWAYS AS (
        COALESCE(monthly_hoa * 12, 0) + COALESCE(estimated_annual_taxes, 0) +
        COALESCE(estimated_insurance, 0) + COALESCE(estimated_utilities, 0) +
        COALESCE(property_management_fee, 0)
    ) STORED,
    net_operating_income NUMERIC(12,2) GENERATED ALWAYS AS (
        COALESCE(annual_rent, 0) - total_annual_expenses
    ) STORED,
    cap_rate NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN current_price > 0 THEN ROUND(net_operating_income / current_price * 100, 2) ELSE 0 END
    ) STORED,
    gross_rent_multiplier NUMERIC(5,1) GENERATED ALWAYS AS (
        CASE WHEN annual_rent > 0 THEN ROUND(current_price / annual_rent, 1) ELSE 0 END
    ) STORED,
    loan_amount NUMERIC(12,2) GENERATED ALWAYS AS (current_price - COALESCE(down_payment, 0)) STORED
);
...
CREATE TABLE comparable_sales (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    ppsf NUMERIC(10,2) GENERATED ALWAYS AS (
        CASE WHEN sqft > 0 THEN ROUND(sale_price / sqft, 2) ELSE 0 END
    ) STORED,
    prior_year_ppsf NUMERIC(10,2) GENERATED ALWAYS AS (
        CASE WHEN sqft > 0 AND prior_year_sale_price > 0
        THEN ROUND(prior_year_sale_price / sqft, 2) ELSE 0 END
    ) STORED,
    yoy_ppsf_change NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN prior_year_ppsf > 0
        THEN ROUND((ppsf - prior_year_ppsf) / prior_year_ppsf * 100, 2) ELSE 0 END
    ) STORED,
    yoy_price_change NUMERIC(5,2) GENERATED ALWAYS AS (
        CASE WHEN prior_year_sale_price > 0
        THEN ROUND((sale_price - prior_year_sale_price) / prior_year_sale_price * 100, 2) ELSE 0 END
    ) STORED
);
...
SELECT p.*, n.neighborhood_name, n.borough,
...
FROM properties p
LEFT JOIN neighborhoods n ON p.neighborhood_id = n.id
WHERE p.status = 'Active' AND p.cap_rate >= 3.5
  AND p.gross_rent_multiplier > 0 AND p.gross_rent_multiplier <= 15
