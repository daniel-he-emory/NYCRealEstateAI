-- NYC Real Estate AI - Supabase Schema Migration
-- Generated from Airtable schema conversion
-- Includes all 6 tables with computed columns, indexes, and relationships
...
CREATE TABLE neighborhoods (
    ...
    price_change_yoy NUMERIC(5,2), -- stored as percentage
...
CREATE TABLE properties (
    ...
    price_history JSONB, -- stores array of {date, price}
    ...
    property_tags TEXT[], -- array of tags
    ...
    interest_rate NUMERIC(5,4), -- stored as decimal (e.g., 0.065 for 6.5%)
...
-- Computed columns for Properties
...
-- Note: building_age removed - calculate in application layer due to CURRENT_DATE immutability
-- Use: EXTRACT(YEAR FROM CURRENT_DATE)::INTEGER - year_built in queries
...
ALTER TABLE properties ADD COLUMN cap_rate NUMERIC(5,2)
    GENERATED ALWAYS AS (
        CASE
            WHEN current_price > 0 THEN ROUND(net_operating_income / current_price * 100, 2)
            ELSE 0
        END
    ) STORED;
...
ALTER TABLE properties ADD COLUMN gross_rent_multiplier NUMERIC(5,1)
    GENERATED ALWAYS AS (
        CASE
            WHEN annual_rent > 0 THEN ROUND(current_price / annual_rent, 1)
            ELSE 0
        END
    ) STORED;
...
CREATE TABLE comparable_sales (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    bbl TEXT NOT NULL, -- Borough-Block-Lot
    address TEXT NOT NULL,
    building_name TEXT,
    unit_number TEXT,
    bedrooms INTEGER,
    ...
    neighborhood_id UUID REFERENCES neighborhoods(id),
    property_link UUID REFERENCES properties(id), -- link to main property if this is a comp
...
ALTER TABLE comparable_sales ADD COLUMN yoy_ppsf_change NUMERIC(5,2)
    GENERATED ALWAYS AS (
        CASE
            WHEN prior_year_ppsf > 0 THEN ROUND((ppsf - prior_year_ppsf) / prior_year_ppsf * 100, 2)
            ELSE 0
        END
    ) STORED;
...
CREATE TABLE historical_sales (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    document_id TEXT UNIQUE NOT NULL, -- ACRIS document ID
    recorded_date DATE,
    address TEXT,
    borough TEXT CHECK (borough IN ('Manhattan', 'Brooklyn', 'Queens', 'Bronx', 'Staten Island')),
    neighborhood_id UUID REFERENCES neighborhoods(id),
...
CREATE TABLE market_metrics (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    date DATE NOT NULL,
    neighborhood_id UUID REFERENCES neighborhoods(id), -- NULL = citywide
...
CREATE TABLE buyer_searches (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    raw_input TEXT,
    parsed_criteria JSONB, -- JSON from AI parser
    beds_min INTEGER,
    beds_max INTEGER,
    baths_min NUMERIC(3,1),
    price_min NUMERIC(12,2),
    price_max NUMERIC(12,2),
    required_amenities TEXT[], -- array of amenities
...
-- COMPLETION MESSAGE
-- ============================================================================
COMMENT ON DATABASE postgres IS 'NYC Real Estate AI - Complete schema with 6 tables, computed columns, and views';
